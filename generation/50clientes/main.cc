		...
		for(pt=0;pt<3;pt++){
			for(t=0;t<2;t++){
				for(od=0;od<1;od++){
					for(p=0;p<1;p++){
						for(f=0;f<1;f++){
							...
							for(j=0;j<prod[p];j++){
								...
								indica = 1;	
								while(aux5 == 0 && indica > 0 ){ //coloquei depois
									cout << " passou aqui  3 " << endl;		
									aux3 = rand()%N;
									if(ntype[aux3]>0){
										iprod[aux3][j]=1;
					...
					NUP[0] = ceil(AJUST*(soma2[0]/indock[f]));
					
					timew[0][0] = a[0];
					timew[0][1] = b[0] + ceil(AJUST*(soma2[0]/indock[f]));
					...
					for(i=1;i<N+1;i++)
							{
								timew[i][0] = a[i] + ceil(AJUST*(soma2[0]/indock[f]));
								timew[i][1] = b[i] + ceil(AJUST*(soma2[0]/indock[f]));	
							}
